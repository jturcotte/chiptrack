...
impl SoundStuff {
    pub fn advance_frame(&mut self) -> () {
        // FIXME: This is ugly, should probably put the instruments in a struct.
        let instruments = &self.instruments;
        let settings_ring = &mut self.settings_ring;
        let settings_ring_index = self.settings_ring_index;
        self.sequencer.advance_frame(|i, freq| {
            instruments[i](settings_ring, settings_ring_index, freq);
        });
    ...
    pub fn trigger_selected_instrument(&mut self, freq: u32) -> () {
        self.trigger_instrument(self.selected_instrument, freq);
    }
}
